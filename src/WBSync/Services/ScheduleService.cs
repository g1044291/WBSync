    ...
    ITaskRepository taskRepo) : IScheduleService
{
        ...
        IReadOnlyDictionary<int, IReadOnlySet<DateOnly>> assigneeHolidays)
    {
        ...
        foreach (var successor in successors)
        {
            ...
            else
            {
                ...
                newEnd = effectiveWorkDays.HasValue
                    ? WorkdayHelper.CalcEndDate(newStart, effectiveWorkDays.Value, globalHolidays, assigneeHolidays, successor.AssigneeId)
                    : (DateOnly?)null;
            }
    ...
    public async Task<HashSet<int>> GetOverlappingTaskIdsAsync(int projectId)
    {
        ...
        foreach (var task in allTasks)
        {
            ...
            var workdays = WorkdayHelper.GetWorkdays(startDate, taskWorkDays.Value, globalHolidays, assigneeHolidays, task.AssigneeId).ToHashSet();
