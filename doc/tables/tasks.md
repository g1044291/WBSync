...
親タスク自身に `work_days`（見積工数）が設定されている場合、直接の子タスクの `work_days` 合計と比較する。
子の合計が親の `work_days` を上回る場合、WBS画面の親タスク行に警告アイコンを表示する（DBへの保存は行わない、表示時の動的判定）。
親タスクの `work_days` が未設定の場合は警告判定を行わない。

## 再計算の伝播

後続タスクの `start_date` 計算には前タスクの `end_date` のみを使用するため、タスク保存時に保存前後で `end_date` に変化がない場合（`start_date` のみの変更を含む）、後続タスクへの再計算処理自体を行わない。
